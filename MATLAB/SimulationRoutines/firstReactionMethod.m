...
while T(rxn_count) < tspan(2)        
    ...
    a  = propensity_fcn(X(rxn_count,:), rate_params,T(rxn_count)); % Minor modification by JB to accomodate for time-varying inputs
...
%     if mod(rxn_count,100) == 0
%         figure(10)
%         plot(T(1:rxn_count),X(1:rxn_count,:))
%         drawnow
%     end
...
if exist('tkeep','var')
    idxs(1) = 1;
    for ind1 = 2:numel(tkeep)
        idxs(ind1) = find(T(1:rxn_count)<tkeep(ind1),1,'last');
    end
    t = tkeep;
    x = X(idxs,:);
else
