...
public class Controller {
	...
	public Controller (double[] ref,double aTcVal,double IPTGVal,BSim sim,double Ts,double kp,double ki, boolean MPC) {
		this.aTcVal=aTcVal;
		this.ref=ref;
	...
	public double Evaluate_Control(double e) {
		...
		if (time>=Lc+Tw) {
			...
			if(MPC) {
				System.out.println("Control Loop n"+index+" at time :"+time+". Next Control at time"+(Lc+2*Tw));
				try {
					double [] DC=eng.feval("bsim_mpc", state_per );
					D=DC[0];
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
	...
	public double Evaluate_Error(Vector<BSimControlledBacterium> bacteria) {
		...
		if(MPC) {
			state_per[0]=bacteria.get(trk_cell).y[2];
			state_per[1]=bacteria.get(trk_cell).y[3];
			state_per[2]=bacteria.get(trk_cell).y[0];
			state_per[3]=bacteria.get(trk_cell).y[1];
			state_per[4]=bacteria.get(trk_cell).y[4];
			state_per[5]=bacteria.get(trk_cell).y[5];
		}else {
